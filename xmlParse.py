                ...
                for schild in child:
                    if schild.tag in ['surrender', 'escape', 'destroyed', 'deadCrew', 'gotaway']:
                        ...
                        if 'load' in schild.attrib:
                            ...
                            ships[child.attrib['name']][schild.tag] = {
                                'load': child.attrib['name'] + '_' + schild.tag.upper()}
        ...
        if child.tag in ['event', 'surrender', 'escape', 'destroyed', 'deadCrew', 'gotaway']:
            ...
            for echild in child:
                ...
                if echild.tag == 'ship':
                    ...
                    if 'load' in echild.attrib:
                        if echild.attrib['load'] in ships:
                            ...
                            print(
                                'ERROR: Unidentified ship ID in a <ship> tag\'s load attribute in event ' + eventNames[
                                    -1])
                    if 'hostile' in echild.attrib:
                        if echild.attrib['hostile'] not in ['false', 'true']:
                            print('ERROR: Hostile attribute in <ship> tag neither "true" nor "false" in event ' +
                                  eventNames[-1])
                        else:
                    ...
                    if 'load' not in echild.attrib and 'hostile' not in echild.attrib:
                        print('ERROR: <ship> tag used in ' + eventNames[
                            -1] + ' event without either a hostile or load tag.')
                ...
                if echild.tag == 'boarders':
                    events[eventNames[-1]]['boarders'] = {'class': echild.attrib['class'],
                                                          'min': int(echild.attrib['min']),
                                                          'max': int(echild.attrib['max'])}
                    ...
                    if int(echild.attrib['amount']) < 1:
                        if echild.attrib['class'] == 'traitor':
                            if echild.attrib['amount'] == '-1':
                                ...
                                print('ERROR: <crewMember> tag has class traitor and has amount that\'s not -1 in ' +
                                      eventNames[-1] + ' event.')
                        else:
                            print('Warning: <crewMember> tag with values 0 or less in ' + eventNames[
                                -1] + ' event. Use the <removeCrew> tag to remove crew members, or class="traitor" to turn a crew member bad.')
                    else:
                        ...
                        if 'all_skills' in events[eventNames[-1]]['crewMember']:
                            if 0 < int(echild.attrib['all_skills']) <= 2:
                                if int(echild.attrib['all_skills']) == 0:
                                    print(
                                        'Advice: Setting a skill to zero in a <crewMember> tag is unnecessary, please remove.')
                                    for skill in skills:
                                        events[eventNames[-1]]['crewMember'][skill] = echild.attrib['all_skills']
                                else:
                                    print('Warning: Skill in a <crewMember> tag in ' + eventNames[
                                        -1] + ' event is not between 0 and 2.')
                            else:
                                for skill in skills:
                                    events[eventNames[-1]]['crewMember'][skill] = echild.attrib['all_skills']
                        else:
                                ...
                                if skill in events[eventNames[-1]]['crewMember']:
                                    if 0 < int(events[eventNames[-1]]['crewMember'].attrib[skill]) <= 2:
                                        if int(events[eventNames[-1]]['crewMember'].attrib[skill]) == 0:
                                            print(
                                                'Advice: Setting a skill to zero in a <crewMember> tag is unnecessary, should be removed.')
                        ...
                        if 'id' in echild.attrib:
                            if echild.attrib['id'] in text_ids:
                                ...
                                print('ERROR: id ' + echild.attrib['id'] + ' not found in ' + eventNames[-1])
                                events[eventNames[-1]]['crewMember']['name'] = [
                                    'id "' + echild.attrib['id'] + '" not found']
                ...
                if echild.tag == 'removeCrew':
                    ...
                    if 'clone' in tagchildren and 'text' in tagchildren:
                        ...
                        if events[eventNames[-1]]['removeCrew']['clone'] not in ['false', 'true']:
                            print('ERROR: <clone> child of <removeCrew> tag in ' + eventNames[
                                -1] + ' event has a value that\'s not true or false.')
                ...
                if echild.tag == 'item_modify':
                    ...
                    for imchild in echild:
                        if imchild.attrib['type'] in ['fuel', 'missiles', 'scrap']:
                            item_modify[imchild.attrib['type']] = {'min': int(imchild.attrib['min']),
                                                                   'max': int(imchild.attrib['max']), 'rand': -1}
                        elif imchild.attrib['type'] == 'drones':
                            item_modify['drone parts'] = {'min': int(imchild.attrib['min']),
                                                          'max': int(imchild.attrib['max']), 'rand': -1}
                        else:
                            print('ERROR: The event ' + eventNames[
                                -1] + ' has an <item> tag with an unrecognisable type attribute.')
                ...
                if echild.tag == 'status':
                    ...
                    if workCheck is True:
                        if echild.attrib['type'] not in ['limit', 'divide', 'loss', 'clear']:
                            ...
                            events[eventNames[-1]]['status'][-1]['amount'] = int(
                                events[eventNames[-1]]['status'][-1]['amount'])
                ...
                if echild.tag == 'upgrade':
                    for attribute in ['system', 'amount']:
                        if attribute not in echild.attrib:
                            ...
                            break
                    events[eventNames[-1]]['upgrade'] = {'system': echild.attrib['system'],
                                                         'amount': int(echild.attrib['amount'])}
                ...
                if echild.tag == 'choice':
                    ...
                    if 'req' in echild.attrib:
                        ...
                        if 'max_lvl' in echild.attrib:
                            events[eventNames[-1]]['choice ' + str(choiceNumber)]['lvl_range'].append(
                                echild.attrib['max_lvl'])
                        else:
                        ...
                        if chochild.tag == 'text':
                            if 'load' in chochild.attrib:
                                ...
                                if chochild.attrib['id'] in text_ids:
                                    events[eventNames[-1]]['choice ' + str(choiceNumber)]['text'] = [
                                        text_ids[chochild.attrib['id']]]
                                else:
            ...
            if 'choice 0' not in events[eventNames[-1]] and 'text' in events[eventNames[-1]]:
                ...
                del events[eventNames[-1]]['removeCrew']
                print('ERROR: <removeCrew> tag not used in ' + eventNames[
                    -1] + ' event with other choice tags. <removeCrew> tag deleted in simulation\'s data.')
        ...
        for choice in range(0, choiceNumb):
            if events[event]['choice ' + str(choice)]['event'] != -1:
                if events[event]['choice ' + str(choice)]['event'] not in eventNames:
                    print('ERROR: Unidentified event ' + events[event]['choice ' + str(choice)][
                        'event'] + ' called by ' + event)
...
def eventEnd(quests):
    ...
    if len(quests) == 0:
        ...
        if len(quests) == 1:
            ...
            print('\n' + str(len(
                quests)) + ' event beacons were placed from this event. do !qload to load a random quest event and !qlist to show a list of loaded quest events. Loading a non-quest event clears this list.')
        rand.shuffle(quests)
        eventMode = False
...
def item_modifyCalc(event):
    if 'item_modify' in events[event]:
        for item in events[event]['item_modify']:
            if item != 'steal':
                events[event]['item_modify'][item]['rand'] = rand.randint(events[event]['item_modify'][item]['min'],
                                                                          events[event]['item_modify'][item]['max'])
...
def noChoiceReq():
    ...
    if 'req' not in events[loadedEvent]['choice ' + str(choiceNumb)] and \
            events[loadedEvent]['choice ' + str(choiceNumb)]['hidden'] == 'false' and 'max_group' not in \
            events[loadedEvent]['choice ' + str(choiceNumb)]:
...
while 0 == 0:
    ...
    if eventMode is False and shipMode is False:
            ...
            for item in events[loadedEvent]['item_modify']:
                if item != 'steal':
                    if int(events[loadedEvent]['item_modify'][item]['rand']) < 0:
                        print('[You lost ' + str(
                            -1 * events[loadedEvent]['item_modify'][item]['rand']) + ' ' + item + ']')
        ...
        if 'upgrade' in events[loadedEvent]:
            simmedEquipment['systems'][events[loadedEvent]['upgrade']['system']] += events[loadedEvent]['upgrade'][
                'amount']
            print('[Your ' + events[loadedEvent]['upgrade']['system'] + 'has been upgraded ' + str(
                events[loadedEvent]['upgrade']['amount']) + ' times]')
        ...
        if 'boarders' in events[loadedEvent]:
            if events[loadedEvent]['boarders']['class'] == 'random':
                print('[' + str(rand.randint(events[loadedEvent]['boarders']['min'], events[loadedEvent]['boarders'][
                    'max'])) + ' boarders have entered your ship]')
            else:
                print('[' + str(rand.randint(events[loadedEvent]['boarders']['min'],
                                             events[loadedEvent]['boarders']['max'])) + ' ' +
                      events[loadedEvent]['boarders']['class'] + ' have entered your ship]')
        ...
        if 'crewMember' in events[loadedEvent]:
            ...
            print('[' + events[loadedEvent]['crewMember']['name'] + ' has joined your crew. They are a ' +
                  events[loadedEvent]['crewMember']['class'] + ']')
        ...
        if 'removeCrew' in events[loadedEvent]:
            if 'class' in events[loadedEvent]['removeCrew']:
                if events[loadedEvent]['removeCrew']['class'] in simmedEquipment['crew']:
                    if events[loadedEvent]['removeCrew']['clone'] != 'true' or 'clonebay' not in simmedEquipment[
                        'systems'].keys():
                        del simmedEquipment['crew'][
                            simmedEquipment['crew'].index(events[loadedEvent]['removeCrew']['class'])]
            ...
            for effect in events[loadedEvent]['status']:
                if effect['target'] == 'player':
                    if effect['type'] == 'clear':
                        ...
                        print('[Your ' + effect['system'] + ' has been ' + limits[effect['type']] + str(
                            effect['amount']) + ' level(s)]')

                else:
                    if effect['type'] == 'clear':
                        print('[Enemy\'s ' + effect['system'] + '\'s status has been cleared.]')
                    else:
                        print('[Enemy\'s ' + effect['system'] + ' has been ' + limits[effect['type']] + str(
                            effect['amount']) + ' level(s)]')
        ...
        if 'ship' in events[loadedEvent]:
            if 'load' in events[loadedEvent]['ship']:
                print('The ship of ID "' + events[loadedEvent]['ship']['load'] + '" is at this beacon. Hostile: ' +
                      events[loadedEvent]['ship']['hostile'])
            ...
            while 0 == 0:
                if 'choice ' + str(choiceNumb) in events[loadedEvent]:
                    ...
                    if 'req' in events[loadedEvent]['choice ' + str(choiceNumb)]:
                        ...
                        for x in range(int(events[loadedEvent]['choice ' + str(choiceNumb)]['lvl_range'][0]),
                                       int(events[loadedEvent]['choice ' + str(choiceNumb)]['lvl_range'][1]) + 1):
                            lvl_range.append(x)

                        if godMode is False:
                            if events[loadedEvent]['choice ' + str(choiceNumb)]['req'] in simmedEquipment[
                                'systems'].keys():
                                if simmedEquipment['systems'][
                                    events[loadedEvent]['choice ' + str(choiceNumb)]['req']] not in lvl_range:
                                    noChoiceReq()
                                    continue

                            elif events[loadedEvent]['choice ' + str(choiceNumb)]['req'] in simmedEquipment['cargo']:
                                if list(simmedEquipment['cargo']).count(
                                        events[loadedEvent]['choice ' + str(choiceNumb)]['req']) not in lvl_range:
                                    noChoiceReq()
                                    continue

                            elif list(simmedEquipment['crew']).count(
                                    events[loadedEvent]['choice ' + str(choiceNumb)]['req']) not in lvl_range:
                                noChoiceReq()
                                continue
                    ...
                    if eventCheck != -1:
                            ...
                            if godMode is False:
                                if events[eventCheck]['item_modify']['steal'] == 'false':
                                    ...
                                    for item in events[eventCheck]['item_modify']:
                                        if item != 'steal':
                                            if -1 * events[eventCheck]['item_modify'][item]['rand'] > simmedEquipment[
                                                item]:
                                                reqCheck = False
                                                break
                        ...
                        if events[loadedEvent]['choice ' + str(choiceNumb)]['hidden'] == 'false':
                            if 'item_modify' in events[eventCheck]:
                                for item in events[eventCheck]['item_modify']:
                                    if item != 'steal':
                                        choiceEffectText += str(
                                            events[eventCheck]['item_modify'][item]['rand']) + ' ' + item + '; '

                            if 'cargoAdd' in events[eventCheck]:
                                choiceEffectText += 'Item: ' + events[eventCheck]['cargoAdd'] + '; '

                            if 'crewMember' in events[eventCheck]:
                                choiceEffectText += 'Crew: ' + events[eventCheck]['crewMember']['name'] + ', ' + \
                                                    events[eventCheck]['crewMember']['class'] + '; '
